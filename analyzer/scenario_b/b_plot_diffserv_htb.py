...
def main():
    ...
    _bar_plot(labels, thr_means,
              "Shaping (Scenario B)", "Throughput (RX) [Mb/s]",
              os.path.join(out_dir, "b_throughput_rx_ci.png"),
              ylim=tuple(args.ylim_throughput),
              yerr=thr_cis)

    _bar_plot(labels, loss_means,
              "Shaping (Scenario B)", "Packet loss [%]",
              os.path.join(out_dir, "b_loss_rx_ci.png"),
              ylim=tuple(args.ylim_loss),
              yerr=loss_cis)

    _bar_plot(labels, jit_means,
              "Shaping (Scenario B)", "Jitter [ms]",
              os.path.join(out_dir, "b_jitter_rx_ci.png"),
